...
docker buildx build --no-cache --progress=plain --push --platform linux/amd64,linux/arm64/v8 --rm -t wujidadi/rocky-linux-tuned:3.8 -t wujidadi/rocky-linux-tuned:latest . 2>&1 | tee $D/docker-build-rlt-3.8.log

# Create testing container
docker run -d -p 50000:80 -it --name Test wujidadi/rocky-linux-tuned:3.8
